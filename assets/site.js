...
var T = EN ? {
  ...
  si: 'Yes', no: 'No', una: '1 company found', varias: ' companies found'
} : {
  abrir: 'Abrir menú', cerrar: 'Cerrar menú', verDic: 'Ver en el diccionario',
  si: 'Sí', no: 'No', una: '1 compañía encontrada', varias: ' compañías encontradas'
};
...
// Formularios (contacto, cita y presupuesto): componen un correo con los
// datos introducidos. El sitio es estático, así que no hay servidor donde
// enviarlo. Cada campo aporta su etiqueta visible al cuerpo del mensaje.
(function () {
  document.querySelectorAll('form[data-to]').forEach(function (form) {
    form.addEventListener('submit', function (e) {
      ...
      Array.prototype.forEach.call(form.elements, function (el) {
        if (!el.name || el.type === 'submit' || el.name === 'consent') return;
        var etiqueta = el.dataset.label || el.name;
        var valor = el.type === 'checkbox' ? (el.checked ? T.si : T.no) : el.value.trim();
        if (el.name === 'mensaje') { mensaje = valor; return; }
