...
SERVICIOS = {}
for _m in ("servicios_chapa", "servicios_mecanica", "servicios_otros"):
...
def opciones_servicio(lang):
    return [t(lang, "Chapa y pintura", "Bodywork & paint"), t(lang, "Mecánica y mantenimiento", "Mechanics & servicing"),
            t(lang, "Diagnosis de una avería", "Fault diagnosis"), t(lang, "Revisión pre-ITV", "Pre-ITV check"),
            t(lang, "Vehículo eléctrico o híbrido", "Electric or hybrid vehicle"),
            t(lang, "Reparación a cargo del seguro", "Insurance repair"),
...
def pagina_inicio(lang):
  ...
  <span class="eyebrow">{L("Chapa · Pintura · Mecánica · Las Chafiras, Tenerife", "Bodywork · Paint · Mechanics · Las Chafiras, Tenerife")}</span>
  <h1>{L("Tu taller de chapa, pintura y mecánica en el sur de Tenerife", "Your bodywork, paint and mechanics garage in the south of Tenerife")}</h1>
  <p class="lead">{md(L("Desde 2019 ofrecemos un servicio integral y sostenible: reparamos tu coche con [[pintura-al-agua|pintura ecológica]], trabajamos con tu compañía de seguros, recogemos y entregamos el vehículo a domicilio y te mantenemos informado en tiempo real.", "Since 2019 we have offered a complete, sustainable service: we repair your car with [[pintura-al-agua|eco-friendly paint]], work with your insurance company, collect and deliver your vehicle and keep you updated in real time."), lang)}</p>
  <div class="actions">
...
"""
    cats = [
        ("spray", "chapa-y-pintura.html", L("Chapa y pintura", "Bodywork & paint"),
         L("Golpes, abolladuras, reparación tras accidente, bancada, pintura completa o parcial y acabados metalizados.", "Dents, accident repair, chassis jig, full or partial resprays and metallic finishes.")),
        ("wrench", "mecanica.html", L("Mecánica", "Mechanics"),
         L("Mantenimiento, frenos, suspensión, embrague, distribución, aire acondicionado y revisión pre-ITV.", "Servicing, brakes, suspension, clutch, timing belt, air conditioning and pre-ITV checks.")),
        ("chip", "diagnosis-y-electronica.html", L("Diagnosis y electrónica", "Diagnostics & electronics"),
         L("Equipos de diagnosis para localizar cualquier avería, electricidad, baterías y testigos del cuadro.", "Diagnostic equipment to track down any fault, electrics, batteries and dashboard warning lights.")),
        ("bolt", "electricos-e-hibridos.html", L("Eléctricos e híbridos", "Electric & hybrid"),
         L("Reparación y mantenimiento de vehículos eléctricos e híbridos, también de chapa y pintura.", "Repair and servicing of electric and hybrid vehicles, including bodywork and paint.")),
    ]
    s1 = seccion(section_head(L("¿Qué necesita tu coche?", "What does your car need?"),
                              L("Todos nuestros servicios, organizados por especialidad. En cada uno te explicamos qué hacemos y cómo trabajamos.", "All our services, organised by speciality. Each page explains what we do and how we work."), lang)
                 + grid("g4", [card_ico(i, a, b, lang, h, L("Ver servicios", "See services")) for i, h, a, b in cats]))
...
""", "alt")
    ...
    {checks_titulados([
        ...
        (L("Y después, tu taller", "And afterwards, your garage"), L("Mantenimiento, chapa y mecánica en el mismo sitio.", "Servicing, bodywork and mechanics in one place.")),
    ], lang)}
  </div>
""", "alt")
    cuerpo = hero + s1 + s2 + s3 + s4 + s5 + s6 + s7 + mitos(lang, "") + panel(
    ...
    return documento(lang, "index", L("Green Car Service Tenerife | Taller de chapa, pintura y mecánica en Las Chafiras", "Green Car Service Tenerife | Bodywork, paint & mechanics garage in Las Chafiras"),
                     L("Taller de chapa, pintura y mecánica en Las Chafiras (sur de Tenerife). Pintura ecológica, garantía vitalicia en pintura, recogida y entrega a domicilio y reparación de eléctricos.", "Bodywork, paint and mechanics garage in Las Chafiras (south Tenerife). Eco-friendly paint, lifetime paint warranty, collection and delivery, and electric vehicle repair."),
                     cuerpo, "index")
...
def pagina_conocenos(lang):
    ...
    <div>
      ...
      <p style="color:var(--muted);font-size:18px">{L("Chapa, pintura y mecánica bajo el mismo techo, en Las Chafiras.", "Bodywork, paint and mechanics under one roof, in Las Chafiras.")}</p>
      <p style="color:var(--muted)">{md(L("Nacimos con el objetivo de ofrecer un servicio integral: que puedas dejar el coche en un solo sitio, sea un golpe, una revisión o una avería, y recuperarlo reparado, limpio y con todas las explicaciones. Y hacerlo de forma sostenible, con [[pintura-al-agua|pintura ecológica]] y procesos que cuidan el entorno.", "We were set up to offer a complete service: you leave your car in one place, whether it's a dent, a service or a fault, and get it back repaired, clean and fully explained. And to do it sustainably, with [[pintura-al-agua|eco-friendly paint]] and processes that respect the environment."), lang)}</p>
    </div>
    <div class="hero-stats" style="margin-top:0">
      ...
      <div class="card"><b style="font-size:26px;color:var(--green-dark)">3</b><br><span style="color:var(--muted)">{L("especialidades: chapa, pintura y mecánica", "specialities: bodywork, paint, mechanics")}</span></div>
...
""", "alt")
    cuerpo += seccion(section_head(L("Nuestros tres pilares", "Our three pillars"),
                      ...
                      + grid("g3", [
                          card_ico("award", L("Calidad", "Quality"), L("Instalaciones y equipos para trabajar con los estándares que exige cada marca: bancadas, diagnosis electrónica y área de pintura.", "Premises and equipment to work to each manufacturer's standards: chassis jigs, electronic diagnostics and a paint area."), lang),
    ...
    cuerpo += seccion(grid("g3", [
        card_ico("tool", L("Nuestras instalaciones", "Our premises"), L("Taller de chapa, pintura, almacén de recambios y equipos de diagnosis en el polígono Llano del Camello.", "Bodywork and paint shop, parts store and diagnostic equipment on the Llano del Camello industrial estate."), lang, "instalaciones.html"),
    ...
    return documento(lang, "conocenos", L("Conócenos", "About us"),
                     L("Green Car Service Tenerife: taller de chapa, pintura y mecánica en Las Chafiras desde 2019. Calidad, confianza y compromiso.", "Green Car Service Tenerife: bodywork, paint and mechanics garage in Las Chafiras since 2019. Quality, trust and commitment."),
                     cuerpo, "conocenos")
...
def pagina_instalaciones(lang):
    ...
    cuerpo += seccion(section_head(L("Qué encontrarás en el taller", "What you'll find at the workshop"), "", lang) + grid("g3", [
        ...
        card_ico("chip", L("Equipos de diagnosis", "Diagnostic equipment"), L("Equipos electrónicos para diagnosticar cualquier avería mecánica o eléctrica.", "Electronic equipment to diagnose any mechanical or electrical fault."), lang),
        ...
        card_ico("bolt", L("Vehículos eléctricos", "Electric vehicles"), L("Reparamos también vehículos eléctricos e híbridos.", "We also repair electric and hybrid vehicles."), lang, "electricos-e-hibridos.html"),
    ]))
...
""", "alt")
    ...
    return documento(lang, "instalaciones", L("Instalaciones", "Our premises"),
                     L("Instalaciones de Green Car Service Tenerife en Las Chafiras: taller de chapa, pintura, bancadas, diagnosis electrónica y almacén de recambios.", "Green Car Service Tenerife premises in Las Chafiras: body shop, paint shop, chassis jigs, electronic diagnostics and parts store."),
                     cuerpo, "conocenos")
...
def pagina_sostenibilidad(lang):
    ...
    cuerpo += seccion(section_head(L("Cómo lo llevamos a la práctica", "How we put it into practice"), "", lang) + grid("g2", [
        ...
        card_ico("recycle", L("Residuos bien gestionados", "Waste handled properly"), L("Aceites, filtros, baterías, disolventes y envases de un taller necesitan un tratamiento específico. Los separamos y los entregamos a gestores autorizados.", "Oils, filters, batteries, solvents and containers from a workshop need specific treatment. We separate them and hand them to authorised waste managers."), lang),
        card_ico("bolt", L("Preparados para el eléctrico", "Ready for electric"), L("Reparamos y mantenemos vehículos eléctricos e híbridos, para que la movilidad de bajas emisiones tenga también su taller de confianza.", "We repair and service electric and hybrid vehicles, so low-emission mobility has a trusted garage too."), lang, "electricos-e-hibridos.html"),
    ]))
    cuerpo += seccion(f"""  <div class="texto-largo">
    <h2>{L("Mantenimiento que también es sostenible", "Maintenance that is sustainable too")}</h2>
    <p>{md(L("Un coche bien mantenido contamina menos y dura más. Un [[filtro-de-aire|filtro de aire]] limpio, la presión correcta de los [[neumatico|neumáticos]] o un [[filtro-de-particulas|filtro de partículas]] en buen estado ayudan a reducir el consumo y las emisiones. Por eso, en cada revisión te decimos qué conviene cambiar y qué puede esperar.", "A well-maintained car pollutes less and lasts longer. A clean [[filtro-de-aire|air filter]], correct [[neumatico|tyre]] pressures and a [[filtro-de-particulas|particulate filter]] in good condition help reduce fuel consumption and emissions. That's why, at every service, we tell you what should be changed and what can wait."), lang)}</p>
    {checks_simples([L("Te explicamos qué cambiar y por qué.", "We explain what to change and why."), L("No sustituimos piezas que siguen en buen estado.", "We don't replace parts that are still in good condition."), L("Te devolvemos las piezas cambiadas si las quieres ver.", "We give you back the replaced parts if you want to see them.")], lang)}
  </div>
""", "alt")
...
def pagina_servicios(lang):
    ...
    cuerpo = page_head(lang, [], L("Servicios", "Services"),
                       L("Chapa, pintura, mecánica, diagnosis y vehículos eléctricos: todo lo que tu coche necesita, en un solo taller.", "Bodywork, paint, mechanics, diagnostics and electric vehicles: everything your car needs, in a single garage."))
    ...
    return documento(lang, "servicios", L("Servicios", "Services"),
                     L("Todos los servicios de Green Car Service Tenerife: chapa y pintura, mecánica, diagnosis electrónica, eléctricos e híbridos.", "All Green Car Service Tenerife services: bodywork and paint, mechanics, electronic diagnostics, electric and hybrid vehicles."),
                     cuerpo, "servicios")
...
INTRO_CAT = {
    ...
    "mecanica": {"es": "Mantenimiento y reparación mecánica siguiendo el plan de cada fabricante, para que no pierdas la garantía y llegues tranquilo a la [[itv|ITV]].",
                 "en": "Servicing and mechanical repairs following each manufacturer's schedule, so you keep your warranty and face the [[itv|ITV]] with confidence."},
    "diagnosis": {"es": "Equipos electrónicos de diagnosis para localizar cualquier avería, interpretar los [[testigo-de-averia|testigos del cuadro]] y reparar la electricidad del coche.",
                  "en": "Electronic diagnostic equipment to track down any fault, interpret [[testigo-de-averia|dashboard warning lights]] and fix your car's electrics."},
    "electricos": {"es": "Reparamos [[vehiculo-electrico|vehículos eléctricos]] e híbridos: mecánica, mantenimiento y también chapa y pintura, con las precauciones que exige la [[bateria-alta-tension|batería de alta tensión]].",
                   "en": "We repair [[vehiculo-electrico|electric vehicles]] and hybrids: mechanics, servicing and bodywork too, with the precautions a [[bateria-alta-tension|high-voltage battery]] requires."},
...
HUB_TIT = {
    ...
    "diagnosis": ("Diagnosis y electrónica", "Diagnostics & electronics"),
    "electricos": ("Eléctricos e híbridos", "Electric & hybrid"),
    "ventajas": ("Ventajas Green Car", "The Green Car advantage"),
}
HUB_NAV = {"chapa-pintura": "chapa-y-pintura", "mecanica": "mecanica", "diagnosis": "servicios",
           "electricos": "electricos-e-hibridos", "ventajas": "servicios"}
...
def pagina_hub(cat, lang):
    ...
    tarjetas = [card_prod(serv(s, lang)["titulo"], serv(s, lang)["corto"], f"{s}.html", L("Ver servicio", "See service"), lang)
                for s in SERVICIOS_SLUGS[cat] if s in SERVICIOS]
    cuerpo += seccion(section_head(L(f"Todos los servicios de {titulo.lower()}", f"All {titulo.lower()} services") if cat != "ventajas" else L("Todas nuestras ventajas", "All our advantages"),
                      ...
                      + grid("g3", tarjetas))
    cuerpo += bloque_aseguradora(lang, cat != "ventajas")
    if cat != "ventajas":
...
def pagina_servicio(s, lang):
    ...
    cuerpo += seccion(section_head(L(f"Otros servicios de {cat_nombre(cat, lang).lower()}", f"Other {cat_nombre(cat, lang).lower()} services") if cat != "ventajas" else L("Otras ventajas Green Car", "More Green Car advantages"), "", lang)
                      + grid("g3", tarjetas)
...
""".replace("</a>\n    <div", f" {ico('arrow')}</a>\n    <div", 1), "alt" if cat == "ventajas" else "")
    ...
    return documento(lang, s, C["titulo"], C["entradilla"], cuerpo, HUB_NAV[cat])
...
def pagina_telefonos(lang):
    ...
    cuerpo = page_head(lang, [(L("Siniestros", "Accidents"), "siniestros.html")], L("Teléfonos de asistencia en carretera", "Roadside assistance numbers"),
                       L("Si tu coche se queda parado o has tenido un accidente, el primer paso es llamar a la asistencia de tu compañía. Aquí tienes los teléfonos de autos de las principales aseguradoras.", "If your car breaks down or you've had an accident, the first step is to call your insurer's assistance line. Here are the motor numbers of the main insurers."))
    fichas = []
...
""")
    ...
    return documento(lang, "telefonos-de-asistencia", L("Teléfonos de asistencia en carretera", "Roadside assistance numbers"),
                     L("Teléfonos de asistencia en carretera y de autos de las principales aseguradoras.", "Roadside assistance and motor claims numbers of the main insurers."), cuerpo, "siniestros")
...
def pagina_ocasion(lang):
    ...
    {checks_titulados([
        ...
        (L("Mantenimiento después", "Servicing afterwards"), L("Chapa, pintura y mecánica en el mismo taller.", "Bodywork, paint and mechanics at the same garage.")),
...
def pagina_blog(lang):
    ...
    return documento(lang, "blog", "Blog", L("Blog de Green Car Service Tenerife: mantenimiento, chapa y pintura, eléctricos, seguridad vial, seguros y conducir en Tenerife.", "Green Car Service Tenerife blog: maintenance, bodywork and paint, electric cars, road safety, insurance and driving in Tenerife."), cuerpo, "blog")
...
def pagina_diccionario(lang):
    ...
    cuerpo = page_head(lang, [], L("Diccionario del motor", "Motoring glossary"),
    ...
    return documento(lang, "diccionario-del-motor", L("Diccionario del motor", "Motoring glossary"),
                     L("Glosario de términos de taller, mecánica, chapa, pintura y seguros del automóvil explicados en lenguaje claro.", "Glossary of garage, mechanics, bodywork, paint and car insurance terms explained in plain language."), cuerpo, "blog")
...
def pagina_faq(lang):
    ...
    return documento(lang, "preguntas-frecuentes", L("Preguntas frecuentes", "Frequently asked questions"),
                     L("Preguntas frecuentes sobre citas, reparaciones, seguros, vehículos eléctricos y coches de ocasión en Green Car Service Tenerife.", "FAQ about appointments, repairs, insurance, electric vehicles and used cars at Green Car Service Tenerife."), cuerpo, "contacto")
...
""")
    ...
    return documento(lang, "presupuesto", L("Presupuesto sin compromiso", "Free estimate"), L("Pide presupuesto sin compromiso para chapa, pintura o mecánica en Green Car Service Tenerife.", "Ask for a free, no-obligation estimate for bodywork, paint or mechanics at Green Car Service Tenerife."), cuerpo, None)
...
def pagina_mapa(lang):
    ...
    nombres = {"principal": L("Principales", "Main pages"), "chapa-pintura": L("Chapa y pintura", "Bodywork & paint"), "mecanica": L("Mecánica", "Mechanics"),
               "diagnosis": L("Diagnosis y electrónica", "Diagnostics & electronics"), "electricos": L("Eléctricos e híbridos", "Electric & hybrid"),
...
def main():
    ...
    P("diccionario-del-motor", pagina_diccionario, "Diccionario del motor", "Motoring glossary", "principal")
